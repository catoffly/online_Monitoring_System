...
//*******************************************************************************************
/* CRC余式表 */
const static  u8 crc_ta_Array[]={0x0, 0x0, 0x10, 0x21, 0x20, 0x42, 0x30, 0x63, 0x40, 0x84, 0x50, 0xA5, 0x60, 0xC6, 0x70, 0xE7, 0x81, 0x8, 0x91, 0x29, 0xA1, 0x4A, 0xB1, 0x6B, 0xC1, 0x8C, 0xD1, 0xAD, 0xE1, 0xCE, 0xF1, 0xEF,
...
//----------------------------------------------------------------------------
//计算长度为bytelength的byte型数组CRCBytes()的CRC
long yb_crc(u8* CRCBytes,int bytelength )
{       //BYTE crc_ta[512];     
        u8 crc[2];
        int i, j, k ;
        i = j = k = 0;
        ...
        while (bytelength != 0)
        {
                ...
                crc[1] = 0;
                i = k ^ CRCBytes[j];
                crc[0] = crc[0] ^ crc_ta_Array[i * 2];
                crc[1] = crc_ta_Array[i * 2 + 1];
                j = j + 1;
        }
...
//        LPBYTE lpGISBuff = NULL;
//        int nBuffLen=512;
//        lpGISBuff = new BYTE[nBuffLen];
//        memset(lpGISBuff,1,nBuffLen);
//        //假设要发送的lpGISBuff内容为fwq123
//        memcpy(lpGISBuff,"fwq123",6);
//        //求lpGISBuff的CRC码
//        long crc_code;
//        crc_code = yb_crc(lpGISBuff,6);

//        printf("The crc of lpGISBuff is %d/n", crc_code);
//        //将CRC码放在要发送信息之后
//        //可以在此处加入便于于接收方识别的帧尾
//        sprintf((char*)(lpGISBuff+6),"%d",crc_code);
//        printf("before add head lpGISBuff : %s/n", lpGISBuff);

//        //计算加入CRC码后lpGISBuff的长度
//        for(int i=0;lpGISBuff[i]!=NULL;i++);
//        printf("The len of lpGISBuff is %d /n", i);
//        //为加入CRC码后字符串增加一个帧头，用于接收方识别。
//        memcpy(g_GisData,"SKGIS:",6);//加入帧头
//        memcpy(g_GisData+6,lpGISBuff,i+1);
//        printf("The g_GisData is %s/n", g_GisData);
